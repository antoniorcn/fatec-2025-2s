...
class ContatoController( val servico : ContatoServico) {
    ...
    fun lerContatos(@RequestParam("nome") nome : String?) : ResponseEntity<List<Contato>> {
        if (nome != null) {
            return ResponseEntity.ok(servico.consultarPorNome( nome ))
        } else {
            return ResponseEntity.ok(servico.lerTodos())
        }
    }

    /*
    * http://localhost:8080/contato?id=<numero id>   // Por parametro (RequestParam)
    * fun apagarContato(@RequestParam("id") id : Int) : ResponseEntity<String> {
    *
    * http://localhost:8080/contato/<numero id>      // Por Path Variable
